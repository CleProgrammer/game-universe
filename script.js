...
function addBox(ni) {
    if(numBox[0] === 0) {
        ...
        newImg.src = '/images/naveInimiga.png'
        ...
        position[numBox2[0]][0] = positionsX[Math.floor(Math.random()*positionsX.length)]
        posBoxPlayerX = position[numBox2[0]][0]
        position[numBox2[0]][1] = -30
        numBox2[0] ++
        numBox[0] ++
        positionAddBox()
    }
    ...
    if(c(`.principal-box .box-add${numBox2[0] - 1}`)) {
        ...
        for(let i = 0; i <= numTimes; i++) {
            if(c(`.principal-box .box-add${i}`)) {
                c(`.principal-box .box-add${i}`).style.marginTop = `${position[i][1]}px`
            }   
        }
    }
}
...
function positionPrincipalBox() {
    c('.principal-box').addEventListener('mousemove', (e)=>{
        ...
        if(e.clientX - 419 > positionMouseX) {
            //console.log( e.clientX + 'desfsdf' )
            if(positionPrincipalBox1[0] >= 420) {
                positionPrincipalBox1[0] = 420
                //console.log('dsfsdf')
            } else {
                positionPrincipalBox1[0] = e.clientX - 470
                //console.log('dfsd')
            }
    ...
    for(let i = 0; i <= numTimes; i++) {
        ...
        if(c(`.principal-box .box-fire${i}`)) {
            for(let k = 0; k <= numTimes; k++) {
                if(checkPositionFire[i][2] - position[k][1] <= 25 && checkPositionFire[i][1] - position[k][0] <= 40 && position[k][0] - checkPositionFire[i][1] <= 30) {
